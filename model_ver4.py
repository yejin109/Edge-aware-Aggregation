...
class HNHNIIConv(nn.Module):
    def __init__(self, in_features, out_features, vidx, eidx, dropout_p ,args):
        ...
        # self.edge2msg = MLP(out_features, out_features, out_features, layer, dropout=dropout_p, Normalization=norm, InputNorm=True)
        ...
        self.edge_denoiser = MLP(out_features+out_features, out_features, out_features, layer, dropout=dropout_p, Normalization=norm, InputNorm=True)
    ...
    def forward(self, v, e, v0, e0, alpha, beta):
        ...
        # edge = (1-alpha) * edge + alpha *e0
        # edge = edge + self.edge_denoiser(torch.cat((e0, e), -1))
        ...
        # edge = scatter(node_msg, self.eidx, 0, reduce= 'mean')
        # edge = .5*((1-alpha) * edge + alpha *e0) + .5 * e
        # edge_msg = beta * self.edge_agg(edge) + (1-beta) * edge

        edge_msg = edge[self.eidx]   

        # edge_msg = self.edge2msg(edge * self.e_weight)
        # edge_msg = edge_msg[self.eidx]
        # edge_msg *= self.e_reg_weight

        node = scatter(edge_msg, self.vidx, dim=0, reduce='mean', dim_size=v.size(0))
        # node *= self.n_weight

        # node = scatter(edge_msg, self.vidx, dim=0, reduce='sum', dim_size=v.size(0))
        # node /= self.n_reg_sum
...
class Hypergraph(nn.Module):
    ...
    def __init__(self, vidx, eidx, ne, args):
        ...
        # self.cls = MLP((self.args.n_hidden+self.args.n_hidden), self.args.n_hidden//2, self.args.n_cls, layer, dropout=dropout, Normalization=norm, InputNorm=False)
    ...
    def forward(self, v, e):
        ...
        # e0 = scatter(v0[self.vidx], self.eidx, 0, reduce= 'mean')
